...
function isExpectedOrder(order, from, payment) {
  return Boolean(
    order
      && String(order.user_id) === String(from.id)
...
async function sendInvoice(message) {
  if (message.chat.type !== 'private') {
    await telegram('sendMessage', {
      ...
      text: 'For privacy, please open a private chat with this bot to purchase.',
    });
    return;
  }
  ...
  const order = await store.create({
    ...
    currency: 'XTR',
    amount: config.product.priceStars,
  });
...
async function handleMessage(message) {
  if (message.successful_payment) {
    ...
    const firstReceipt = await store.markPaid({
      payload: order.payload,
    ...
    if (firstReceipt) {
      try {
        ...
        await telegram('sendMessage', {
          chat_id: message.chat.id,
          text: 'Your payment was verified, but delivery needs support. Please send /support and include your Telegram payment receipt.',
        });
      }
    }
    return;
  ...
  if (/^\/terms(?:@\w+)?$/i.test(message.text || '')) {
    return telegram('sendMessage', { chat_id: message.chat.id, text: config.termsText });
  }
  if (/^\/support(?:@\w+)?$/i.test(message.text || '')) {
    return telegram('sendMessage', { chat_id: message.chat.id, text: config.supportText });
  }
  if (/^\/start(?:@\w+)?$/i.test(message.text || '')) {
    return telegram('sendMessage', {
      chat_id: message.chat.id,
      text: 'Welcome! Use the shop link to buy ' + config.product.title + '.',
    });
  }
}
  ...
  if (update.pre_checkout_query) {
    ...
    if (!isExpectedOrder(order, query.from, query)) {
      await telegram('answerPreCheckoutQuery', {
        ...
        ok: false,
        error_message: 'This order is unavailable. Please open a new invoice from the bot.',
      });
      return;
    }
    ...
    if (!(await deliveryIsAvailable())) {
      await telegram('answerPreCheckoutQuery', {
        pre_checkout_query_id: query.id,
        ok: false,
        error_message: 'This item is temporarily unavailable. Please try again later.',
      });
      return;
    }
    ...
    await telegram('answerPreCheckoutQuery', approved
      ...
      : {
          pre_checkout_query_id: query.id,
          ok: false,
          error_message: 'This order cannot be processed.',
        });
    return;
  }
...
function serveStatic(response, pathname) {
  ...
  if (
    (!filePath.startsWith(permittedPrefix) && filePath !== siteRoot)
    || !existsSync(filePath)
  ...
  try {
    ...
    if (request.method === 'GET' && url.pathname === '/api/public-config') {
      return sendJson(response, 200, {
        ...
        product: {
          key: config.product.key,
          title: config.product.title,
          description: config.product.description,
          priceStars: config.product.priceStars,
        },
      });
    }
...
server.listen(config.port, () => {
  console.log('Shop server listening on port ' + config.port);
});
